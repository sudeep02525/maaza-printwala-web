...
const PRINTING_NEEDS = [
  { name: 'Marketing Events', icon: '🎪', link: '/all?need=marketing' },
  { name: 'Office Stationery', icon: '📎', link: '/all?need=office' },
  { name: 'Product Packaging', icon: '📦', link: '/all?need=packaging' },
  { name: 'Restaurant Menus', icon: '🍔', link: '/all?need=restaurant' },
  { name: 'Corporate Gifting', icon: '🎁', link: '/all?need=corporate' },
  { name: 'Wedding Invites', icon: '💌', link: '/all?need=wedding' },
];
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {POPULAR_SEARCHES.map((term, i) => (
                <Link key={i} href={`/all?search=${encodeURIComponent(term)}`}>
                  <motion.div 
